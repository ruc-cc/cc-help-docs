...
为了满足全校师生复杂多样的个性化计算需求，适应新型计算模式如人工智能训练等对运行环境的复杂要求，校级计算平台平台的建设除了在硬件设备的选型上充分考虑了计算的多样性，提供了包含普通x86节点，x86胖节点（大内存）、Tesla V100/A100/A800 GPU节点、GeForce GPU节点等丰富算力组合，在软件平台的建设上也采用了区别于传统高性能计算中心的作业调度系统的解决方案，在不损失计算性能的前提下以灵活多样的方式提供服务。
...
| 速度     | 快                              | 快                                                                            | 慢                                                                            | 慢                                                                            |
| 应用类型   | 绝大多数计算任务：深度学习等                 | Jupyter、RStudio等有交互界面；MATLAB、Stata的Linux版本。                                  | 徒手搭建的软件或系统                                                                   | Windows上的SAS、Stata等所有软件。                                                     |
