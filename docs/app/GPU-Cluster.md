...
目前，我们使用的是一个多因子（Multi-Factor）排队规则。对于队列中排队的作业，Slurm会综合多项因子来计算每个作业的优先级，因子包括排队时间、QOS、作业大小等等。

考虑到有些用户在某个特定时间（论文提交、项目结题）需要进行大量计算，因此用户可以通过配置QOS来提高本作业的优先权，以便排到队列最前面。

QOS(Quality of Service)是Slurm提供的一种服务质量功能，可以对不同类型的作业提供不同的服务。不同的QOS对作业有不同的限制，比如最长运行时间、单个用户同时最多可占用的CPU、GPU数量等。目前，默认的QOS如下：

| QOS名称 | 描述                                                         | 最长运行时间          | CPU、GPU数量                      | 计费权重 |
| ------- | ------------------------------------------------------------ | --------------------- | --------------------------------- | -------- |
| normal  | 默认所有用户使用此规则                                       | CPU队列7天 GPU队列5天 | 每个用户同时使用的GPU卡上限为8个  | 1        |
| gpu     | 有紧急GPU需求的用户请使用此规则，保证优先级在其他所有排队作业中最高 | GPU队列5天            | 每个用户同时使用的GPU卡上限为16个 | 4        |
| cpu     | 有紧急CPU需求的用户请使用此规则，保证优先级在其他所有排队作业中最高 | CPU队列7天            |                                   | 2        |

### 使用方法

在`salloc`或`sbatch`中使用`--qos=<qos_name>`来选择所使用的QOS。